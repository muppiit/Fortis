...
<head>
    ...
    <title>Daftar Team Department</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        @if (in_array(Auth::user()->role->level, ['super_super_admin', 'super_admin']))
            <a href="{{ route('admin.team_departments.create') }}" class="btn btn-primary">+ Tambah Team Department</a>
        @endif
                ...
                <tbody>
                    @forelse($teamDepartments as $team)
                        @if (Auth::user()->role->level === 'super_super_admin' ||
                                (Auth::user()->role->level === 'super_admin' &&
                                    $team->department_id === Auth::user()->teamDepartment->department_id))
                            <tr>
                                <td>{{ $team->id }}</td>
                                <td>{{ $team->name }}</td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <form action="{{ route('admin.team_departments.destroy', $team->id) }}"
